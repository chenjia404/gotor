...
func TestHandleLinkedNonceAndAck(t *testing.T) {
	...
	handled, err := s.onRelayCell(a, rc)
	if err != nil || !handled {
	...
	if _, err := s.onRelayCell(a, badRC); err == nil {
...
func TestConfluxReorderAndSwitch(t *testing.T) {
	...
	if _, err := s.onRelayCell(b, sw); err != nil {
		t.Fatal(err)
	}
	late, _ := cell.NewRelayCell(7, cell.RelayData, []byte("second"))
	if _, err := s.onRelayCell(b, late); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.onRelayCell(a, first); err != nil {
		t.Fatal(err)
	}
