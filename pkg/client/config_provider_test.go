...
func TestClientConfigProvider_GetConfigValue(t *testing.T) {
	...
	tests := []struct {
		...
		{"ClientUseIPv4", "ClientUseIPv4", "1", true},
		{"ClientUseIPv6", "ClientUseIPv6", "0", true},
		{"ClientPreferIPv6ORPort", "ClientPreferIPv6ORPort", "1", true},
		{"ClientRejectInternalAddresses", "ClientRejectInternalAddresses", "1", true},
...
func TestClientConfigProvider_SetConfigValue(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "ClientUseIPv6 requires restart",
			key:     "ClientUseIPv6",
			value:   "1",
			wantErr: true,
		},
		{
			name:    "ClientRejectInternalAddresses requires restart",
			key:     "ClientRejectInternalAddresses",
			value:   "0",
			wantErr: true,
		},
		{
