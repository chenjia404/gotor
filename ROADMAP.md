...
> 清单第 3 项（DirCache 对外）：上一份→当前 limited-ed（#53）+ gzip/deflate/304（#67）+ FPRLIST（#71）+ x-zstd/x-tor-lzma（#73），**仍 PARTIAL**，未宣告 DirCache=2，无真网被当缓存证据。  
...
> 清单第 10 项（vanguards）：客户端 HS 固定 L2 并落盘（#65），**仍 PARTIAL**（无 L3、无托管侧、无共识 `guard-hs-l2-*`）。第 7 项洋葱托管 INTRODUCE2 是观察/超大项，不要空 PR。第 11 项 Bridge/PT 只改文档。下一项代码缺口：第 3 项剩余（多小时历史 diff）；完整前仍禁止 `DirCache=2`。第 4 项剩余（引言限速 / 会合生命周期）与第 6 项 SENDME v1、第 9 项连接速率也可写，一次一项。
