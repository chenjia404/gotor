...
| `POST /tor/hs/3/publish` | 验 type-8 / 正文签名；revision 只取自签名覆盖范围；按盲化公钥覆盖更高修订（≤100KiB，最多 64 份，3h TTL）；**环就绪后只接受本节点 spread_store 负责的盲化公钥（否则 404）** |
