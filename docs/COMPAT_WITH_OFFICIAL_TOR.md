...
- [ ] **状态**：PARTIAL（digest 交叉引用 + 观测带宽历史已接线，含出站中间跳 OR；**conn-bi-direct 满 24h 才写**；**无** dirreq/exit/hidserv，**无**真网权威归档观察）
...
- **已做（协议切片，2026-09-19）**：`conn-bi-direct` 按 C Tor 10s / 20480 / 10× 分类入站与出站中间跳 OR；满 24h 且该窗有分类才写入 extra-info。不写 `ipv6-conn-bi-direct`。
- **要做**：dirreq / exit / hidserv（需 24h 观测）；`ipv6-conn-bi-direct`；真网权威归档。不要把空统计当完整。
