...
- [ ] **状态**：PARTIAL（客户端 **拉** consdiff 已对齐；中继可 BEGIN_DIR / 明文 DirPort 提供 consensus-microdesc、micro/all、`/tor/keys/fp`、`/tor/keys/all`、**上一份→当前 limited-ed**、**gzip/deflate/`.z`**、**If-Modified-Since 304**。未宣告 DirCache=2；**无**官方客户端把本中继当缓存的证据）
...
- **已做（协议切片，2026-08-20）**：`GenerateConsensusDiff` 产出 limited-ed；换共识保留 `.prev`；匹配摘要则回 diff，否则整份；`/diff/` 未知 404。**#67**：`Accept-Encoding: gzip|deflate`、路径 `.z` 走 zlib、`If-Modified-Since`→304（共识 `Last-Modified` 用 `valid-after`）。描述符 proto **禁止** `DirCache=`。
- **要做（未达 DirCache=2）**：多小时历史 diff；zstd/lzma；按 FPRLIST 过滤；真网被官方客户端当缓存。在此之前 **禁止** 在 `proto` 写 `DirCache=2`。
...
| **目录 / consensus / protover** | 1.4.3 缺协议则退出（[!2929](https://gitlab.torproject.org/tpo/core/arti/-/merge_requests/2929)）；`MicroDesc` 更名为 `Microdesc`。1.5.0 `tor-netdoc` API 大改。2.2.0 consdiff **生成**后端；2.5.0 编解码 router/microdesc/consensus；2.5.1 开始算共识 | 权威+缓存完整；客户端 consdiff=DirCache=2 | 客户端 **WORKING**；对外缓存 **PARTIAL**（上一份→当前 limited-ed + gzip/304；未宣告 DirCache=2） | `pkg/directory/`、`pkg/relay/dirport.go` | 客户端 **P0 已跟**。新投票/共识格式：**P2**，等共识行要求。中继对外 DirCache：**P1**（required-relay；未达完整前禁止写 proto） |
