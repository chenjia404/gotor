...
**客户端**：gotor 主路径已覆盖 recommended-client-protocols（含 HS 客户端 HSDir/HSIntro/HSRend），并额外实现了尚未 required 的 Relay=5/6、Conflux、Padding=2。HS 电路已接客户端 vanguards-lite（固定 L2）。洋葱 PoW 客户端已接线（无真网 PoW 服务验收）。仍缺完整 vanguards（L3 / 托管侧 / 共识参数）与 PT 生产，不是主握手。
...
- [ ] **状态**：PARTIAL（客户端 **拉** consdiff 已对齐；中继可 BEGIN_DIR / 明文 DirPort 提供 **ns（`cached-consensus`）与 microdesc（`cached-microdesc-consensus`）分库**、micro/all、`/tor/keys/fp`、`/tor/keys/all`、**最多 72 小时历史→当前 limited-ed**、**gzip/deflate/`.z`**、**If-Modified-Since 304**、**FPRLIST 过滤权威签名（未过半 404）**、**x-zstd / x-tor-lzma**。未宣告 DirCache=2；**无**官方客户端把本中继当缓存的证据）
...
- **要做（未达 DirCache=2）**：预压缩 diff 库；真网被官方客户端当缓存。在此之前 **禁止** 在 `proto` 写 `DirCache=2`。
