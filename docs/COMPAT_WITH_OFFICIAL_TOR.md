...
- [ ] **状态**：PARTIAL（digest 交叉引用 + 观测带宽历史已接线，含出站中间跳 OR；**conn-bi-direct / ipv6-conn-bi-direct 满 24h 才写**；**dirreq-v3-resp 满 24h 才写**；**无** dirreq ips/reqs/dl、exit/hidserv，**无**真网权威归档观察）
...
- **已做（协议切片，2026-09-19）**：`dirreq-stats-end` / `dirreq-v3-resp` 统计 ns/microdesc 的 200/304/404 等；满 24h 且有计数才写；向上取 4。无 geoip，不写 ips/reqs/dl。
- **要做**：dirreq ips/reqs（需 geoip）、direct/tunneled-dl、exit / hidserv（需 24h 观测）；真网权威归档。不要把空统计当完整。
