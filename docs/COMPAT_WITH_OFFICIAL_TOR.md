...
- [ ] **状态**：PARTIAL（digest 交叉引用 + 观测带宽历史已接线，含出站中间跳 OR；**conn-bi-direct / ipv6-conn-bi-direct 满 24h 才写**；**dirreq-v3-resp / dirreq-v3-ips / dirreq-v3-reqs / dirreq-v3-direct-dl / dirreq-v3-tunneled-dl 满 24h 才写**；ips/reqs 仅 `??`；dl 含 complete/timeout/running；**exit-* 满 24h 且有出口 TCP 才写**；**无** dl 分位数、hidserv，**无**真网权威归档观察）
