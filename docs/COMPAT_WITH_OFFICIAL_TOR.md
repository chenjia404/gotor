...
- [ ] **状态**：PARTIAL（digest 交叉引用 + 观测带宽历史已接线，含出站中间跳 OR；**conn-bi-direct / ipv6-conn-bi-direct 满 24h 才写**；**dirreq-v3-resp / dirreq-v3-direct-dl / dirreq-v3-tunneled-dl 满 24h 才写**；**无** dirreq ips/reqs、dl 分位数、exit/hidserv，**无**真网权威归档观察）
...
- **要做**：dirreq ips/reqs（需 geoip）、dl 分位数、exit / hidserv（需 24h 观测）；真网权威归档。不要把空统计当完整。
