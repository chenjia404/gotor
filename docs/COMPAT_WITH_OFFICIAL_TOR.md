...
| **洋葱托管** | 无（未上线） | ESTABLISH_INTRO；ntor `rend_circ_nonce`；BEGIN_DIR 上传；type-8 致盲证书 + 双层加密密封；torrc `HiddenService*`；**intro/rend 走 vanguards L2+L3**；**HiddenServicePoWDefensesEnabled 写 pow-params 并校验 INTRODUCE2 EXT 0x02**；**ESTABLISH 后常驻收 INTRODUCE2**；上传走 HSDir 哈希环。**真网 ESTABLISH_INTRO 成功；HSDir POST 400** | **真网 HSDir 收 200 后被客户端找到并完成 INTRODUCE2→RENDEZVOUS**；官方 intro/rend 生命周期与限速；prop 362 PoW 控制环 |
...
- [ ] **状态**：PARTIAL（ESTABLISH_INTRO 真网成功；描述符 BEGIN_DIR POST 被 HSDir 回 **400**，未上环；INTRODUCE2 收包已接线但无从验收）
...
- **已做（协议切片，2026-09-20）**：引言电路在发出 ESTABLISH_INTRO 后立刻 `ReceiveRelayCell`（先 INTRO_ESTABLISHED cmd 38，再 INTRODUCE2）；描述符上传走共识 HSDir 哈希环，不再用 Fast+Stable 引言候选当 HSDir；引言点打乱后试到够数；Client 保留并在 Stop 时拆除托管服务。仍缺真网 HTTP 证据。
- **已做（真网观察，2026-09-20）**：独立 ClientOnly 进程（未改公开中继 torrc）对真网引言点完成 ESTABLISH_INTRO → INTRO_ESTABLISHED，三个引言点建立。随后 `/tor/hs/3/publish` 被多个 HSDir 回 HTTP 400，描述符未入环，故无 INTRODUCE2。
- **要做**：修描述符使真网 HSDir 收 200；官方/gotor 客户端访问 → INTRODUCE2 解密 → RENDEZVOUS1 → BEGIN/DATA。写清 onion 地址与 HTTP 状态。
- **禁止**：`TestHandleIntroduce2` 的 mock 字节当 WORKING；明文上传描述符；全零 intro/ntor key。把 ESTABLISH 成功写成托管上线。
...
- [ ] **状态**：PARTIAL（客户端与托管 intro/rend HS 电路固定 L2+L3 并落盘；读共识 `guard-hs-l2-*` / `guard-hs-l3-*`；托管仍未真网上线）
...
- **要做**：真网 PoW 服务 INTRODUCE 被接受；prop 362 控制环（P2，托管未上线前不做）。
