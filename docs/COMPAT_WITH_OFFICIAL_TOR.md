...
- [ ] **状态**：PARTIAL（官方 `DoS*` 键 + CREATE2/每 IP + **auto 跟共识** + **ConnectRate/Burst** + **StreamCreation** + **AUTHENTICATE 单跳区分** + **CircuitCreationDefenseType**）
...
- **已做（协议切片，2026-09-19）**：`DoSRefuseSingleHopClient`：未成功 EXTEND 的普通客户端 DESTROY；入站已 AUTHENTICATE（LinkAuth=3）的中继单跳放行。不查共识是否收录该身份。
...
- **要做**：按共识 nodelist 核对 AUTHENTICATE 身份。
