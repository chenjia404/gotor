...
**客户端接近官方推荐协议；中继实验性（描述符可被权威收下；入站握手 + self-test + 中间跳出站握手已接线，Running / 真网被选为中间跳仍取决于权威与可达性）；洋葱托管真网 HTTP 200 且 PoW v1 已校验（未宣告 HS*）。**
...
| **洋葱** | **有**：托管自 1.2.0（2024-03）起 `onion-service-service` 非实验；2.5.1 增加 AF_UNIX 后端，HS 上的拥塞控制/CGO 仍是实验 feature（`hsc-negotiate-extensions` / `hss-negotiate-extensions`，[#1948](https://gitlab.torproject.org/tpo/core/arti/-/issues/1948)、[#2473](https://gitlab.torproject.org/tpo/core/arti/-/issues/2473)） | gotor 托管已真网 HTTP 200（未宣告 HS*）；不要把 Arti 实验 HS-CGO 开关写成「官方已 required」 |
...
4. **角色差**：Arti 洋葱托管已上线、中继未上线；gotor 洋葱托管已真网 HTTP 200（未宣告 HS*）、中继也未进共识。不能把「Arti 有」直接写成「gotor 该立刻做完」。
...
|------|----------------------|---------|----------------|
| **客户端** | 共识 9/9 验签、`cached-certs` 重启 0 次 `/tor/keys/fp`、DirCache=2 consdiff、microdesc、Link TLS+CERTS type 7、默认 ntor-v3 CREATE2/EXTEND2、3-hop SOCKS5 `IsTor=true`、**HTTPTunnelPort CONNECT `IsTor=true`**、RESOLVE、FlowCtrl=2 Vegas soak、Relay=5/6 CGO、Conflux=1、EXTEND2 IPv6、`p`/`p6` 出口策略、Desc=4 family-ids、Padding=2 协商 ACK、v3 `.onion` 客户端 HTTP 200、**洋葱 PoW Equi-X 求解+INTRODUCE EXT 0x02** | Guard 选路与官方指纹仍可能有差异；Fast/MiddleOnly/BadExit 已强制但未单独真网标 WORKING；circpad token-removal；**vanguards 客户端+托管 intro/rend L2+L3**（读共识 `guard-hs-l*`） | 完整 PT/网桥客户端生产路径；与 Tor Browser 同级的隔离/反指纹 |
| **中继** | 描述符可 POST 到权威并获 HTTP 200；交叉证书（onion-key-crosscert / ntor-onion-key-crosscert）与 Ed25519 摘要签名按 dir-spec 生成；**proto 只宣告已实现的 Cons/Desc/Microdesc/Link/LinkAuth/Relay/FlowCtrl** | ORPort 监听；入站握手 CERTS/AUTH_CHALLENGE/NETINFO；**LinkAuth=3 校验 AUTHENTICATE type 3，AUTH_CHALLENGE 只广告方法 3**；ORPort self-test 门闩（未测活不发布；`AssumeReachable` 跳过探测）；CREATE2 经典 ntor / ntor-v3；**ntor-v3 type 3 `[02 06]` 则 CGO 剥层/回程 + 出口 SENDME v1（16 字节 tag FIFO）**（未宣告 Relay=5-6）；中间跳出站握手（VERSIONS/CERTS/NETINFO）+ CircID MSB + 按身份入池；EXTEND2 剥层转发与回程加密（离线单测）；出口策略解码与 EXIT 流（实验）；DirPort/BEGIN_DIR 可服务 **ns 与 microdesc 分库**、micro/all、`/tor/keys`、**最多 72 小时历史→当前 limited-ed**、**gzip/deflate/`.z` / 304**、**FPRLIST 签名过滤**、**x-zstd / x-tor-lzma**、**预压缩 consdiff 库**（未宣告 DirCache=2）；末端跳 ESTABLISH + **INTRODUCE1→INTRODUCE2 / RENDEZVOUS1→RENDEZVOUS2** + **HSDir `/tor/hs/3` 验签收/服 + 哈希环 spread_store + 引言点令牌桶**（未宣告 HS*）；**extra-info-digest 交叉引用 + 观测带宽历史**（入站 OR + 出站中间跳；IPv6 另写 ipv6-*-history；无观测不写 history）+ **bandwidth observed 来自已完成格**+ **conn-bi-direct / ipv6-conn-bi-direct（满 24h 才写）** + **dirreq-v3-resp / dirreq-v3-ips / dirreq-v3-reqs / dirreq-v3-direct-dl / dirreq-v3-tunneled-dl（满 24h 才写；ips/reqs 仅 ??；dl 含 complete/timeout/running）** + **exit-*（满 24h 才写；BEGIN_DIR 不计）** + **hidserv-v3-*（满 24h 且有会合转发或 HSDir 接受才写；Laplace 混淆；无 v2）**；**官方 DoS* 键 + CREATE2/每 IP + auto 跟共识 + ConnectRate/Burst + StreamCreation + AUTHENTICATE 单跳区分 + CircuitCreationDefenseType + 共识 nodelist 核对身份**（默认 auto 关） | **进共识 `Running`**；真网被官方客户端选为中间跳的证据；对外宣告 DirCache=2（真网被当缓存）；真网被选为 intro/rend/HSDir；HS* proto；真网被请求 CGO 的证据；完整 dos.c（geoip / 其余未接线防御与统计）；完整 extra-info（geoip 国家码、dl 分位数与真网归档） |
| **洋葱托管** | 独立 ClientOnly 进程真网 HTTP 200：`h22s2d4irewv7f7yokwovjd2tsyf3pfwhqyshe3wkjlv32saryvxljyd.onion` 经 SOCKS 返回 200（正文 `gotor-hs-ok`）；HSDir POST 200；ESTABLISH_INTRO→INTRODUCE2→RENDEZVOUS1/2→BEGIN/DATA；**PoW：`pow-params v1` effort=1，客户端 Equi-X 求解，INTRODUCE2 校验通过** | 官方 intro/rend 生命周期与限速；prop 362 PoW 控制环 | HS* proto |
| **网桥 / PT** | 无 | `pkg/pt` 子进程框架、obfs4 配置解析、本地 integration 桩 | 向 BridgeAuth 生产发布；客户端经官方 PT 进网；网桥描述符/统计与 C Tor 对齐 |
| **控制端口** | AUTHENTICATE；**AUTHCHALLENGE SAFECOOKIE**；COOKIE / HASHEDPASSWORD；GETINFO/GETCONF/SETCONF 子集；**GETINFO version 与 PROTOCOLINFO VERSION 对齐 `0.4.9.11 (gotor)`**；**GETINFO traffic/read 与 traffic/written 为入口 OR TLS 累计字节**；**GETINFO status/enough-dir-info 依验签共识与选路表（不写死 1）**；**GETINFO net/listeners/socks、control、httptunnel、dns、or、dir 为实际绑定**；**GETINFO config-file 为 torrc 路径**；**GETINFO config-text 为 dump-config short**；**GETINFO circuit-status 真网 BUILT 行（`$` hex~nick PATH、PURPOSE、TIME_CREATED）**；**GETINFO ns/id 真网 250+ r/s/w（microdesc 8 字段 r + m）**；**GETINFO desc/id 无 server descriptor 缓存时 551**；**GETINFO stream-status** 已接线（SOCKS/HTTP CONNECT 活动流）；**GETCONF CircuitPadding/SafeSocks/ConnectionPadding 为当前值**；**GETCONF ClientUseIPv4/IPv6 与 ClientRejectInternalAddresses 为当前 0/1**；**GETCONF HTTPTunnelPort/DNSPort 为当前端口（未开为 0）**；**GETCONF DisableNetwork/ClientOnly 为当前 0/1**；**GETCONF ORPort/DirPort/Nickname/ExitRelay 为当前中继配置**；**GETCONF ContactInfo/Address/PublishServerDescriptor/AssumeReachable 为描述符发布相关配置**；**GETCONF DirCache/IPv6Exit/ReduceExitPolicy 为当前 0/1（DirCache 是 torrc 开关，不是协议版本 2）**；**GETCONF BandwidthRate/Burst 为字节数（0=不限）**；**GETCONF ExitPolicy 为 torrc 显式规则（不含默认策略展开）**；**GETCONF MyFamily/FamilyID 为当前家族配置**；SETEVENTS（CIRC/STREAM/BW/NOTICE 等）；SIGNAL；MAPADDRESS | GETINFO 键仍少于 control-spec（无 ns/all、md/id）；**stream-status** 已接线（SOCKS/HTTP CONNECT 活动流） | ADD_ONION / DEL_ONION；EXTENDCIRCUIT / ATTACHSTREAM；HSFETCH / HSPOST；USEFEATURE |
...
**客户端**：gotor 主路径已覆盖 recommended-client-protocols（含 HS 客户端 HSDir/HSIntro/HSRend），并额外实现了尚未 required 的 Relay=5/6、Conflux、Padding=2。HS 电路已接客户端与托管 intro/rend 的 vanguards（固定 L2+L3，读共识 `guard-hs-l*`）。洋葱 PoW v1 已真网求解并校验（无 prop 362）。仍缺 PT 生产，不是主握手。
...
问题：缺 **DirCache=2 / HSDir / HSIntro / HSRend**（代码已有切片，缺真网被当缓存 / 被选；hidserv-v3 extra-info 已接线但仍禁止写进 `proto`）；仍发 Link 3。**禁止**在未实现时把 required 行写进 `proto` 骗权威。
...
- [ ] **状态**：PARTIAL（末端跳 ESTABLISH + **INTRODUCE1 转发 INTRODUCE2 / ACK** + **引言点 DOS_PARAMS / 共识 HiddenServiceEnableIntroDoS\* 令牌桶** + **RENDEZVOUS1 会合并拼电路** + **会合点仅末跳 / 未会合 10min TTL** + **HSDir `/tor/hs/3` 验签收/服 + 哈希环 spread_store 责任** + **extra-info hidserv-v3-*（满 24h 且有观测才写，Laplace 混淆）**。未宣告 HS*。**无**真网被选为 intro/rend/HSDir 证据）
- **现有代码**：客户端 `pkg/onion/onion.go`、`pkg/onion/hsdir_index.go`、`pkg/onion/begindir.go`、`pkg/onion/establish_intro.go`、`pkg/onion/hsdir_match.go`。中继 `pkg/relay/hsintro.go`、`pkg/relay/hsrend.go`、`pkg/relay/hsdir.go`、`pkg/relay/hidserv_stats.go`、`pkg/relay/forwarding.go`、`pkg/relay/circuit_handler.go`。互操作 `docs/interop/hs-intro-rend.md`。
- **已做（协议切片，2026-08-20，#69；哈希环 / 引言限速 / 会合生命周期 2026-09-19）**：INTRODUCE1 按 AUTH_KEY 转发；RENDEZVOUS1 cookie 一次性取出后发 RENDEZVOUS2 并拼接。**会合点**：C Tor `n_chan` 语义（已 EXTEND2 则 DESTROY）；未会合 cookie 10 分钟 TTL（TIMEOUT）与最多 128 等待；内存计数 `HSStats`。**哈希环**：读共识 `hsdir_n_replicas` / `hsdir_spread_fetch` / `hsdir_spread_store`；上传用 spread_store；DirCache 在环就绪后拒绝非责任节点的 POST（404）。**引言点限速**：解析 ESTABLISH_INTRO `DOS_PARAMS`（覆盖共识）；否则读 `HiddenServiceEnableIntroDoSDefense`（默认关）/ `RatePerSec=25` / `BurstPerSec=200`；桶空时 ACK `NOT_RECOGNIZED`（对齐 C Tor UNKNOWN_ID），不转发 INTRODUCE2。描述符 proto **禁止** `HSDir=` / `HSIntro=` / `HSRend=`。
- **已做（协议切片，2026-09-20）**：extra-info `hidserv-v3-stats-end` / `hidserv-rend-v3-relayed-cells` / `hidserv-dir-v3-onions-seen`。会合点拼接后每转发一格 RELAY 计数；HSDir 接受的盲化公钥 24h unique。满 24h 且有观测才写；先向上取 bin 再加 Laplace（rend `delta_f=2048 epsilon=0.30 binsize=1024`，dir `delta_f=8 epsilon=0.30 binsize=8`）；噪声在窗旋转时采样一次。无 v2 观测不写 `hidserv-rend-relayed-cells` / `hidserv-dir-onions-seen`。无观测不写。
- **要做（未达 HS* proto）**：真网被选。在此之前 **禁止** 在 `proto` 写 HS*。
...
- [x] **状态**：WORKING（独立 ClientOnly 进程，**未改**公开中继 torrc。描述符 `signature` 无 padding（86 字符）；HSDir `/tor/hs/3/publish` **HTTP 200**。gotor SOCKS 访问 `h22s2d4irewv7f7yokwovjd2tsyf3pfwhqyshe3wkjlv32saryvxljyd.onion` **HTTP 200**，正文 `gotor-hs-ok`。路径：ESTABLISH_INTRO → INTRODUCE2 → RENDEZVOUS1/2（hs-ntor 用引言点 enc-key）→ RELAY_BEGIN/DATA。未宣告 HS*。）
...
- **已做（协议切片，2026-09-20）**：引言电路 ESTABLISH 后常驻收 INTRODUCE2；描述符上传走 HSDir 哈希环；signature RawStdEncoding；外层 10k 填充 + 16 行假 auth-client；会合电路建路前拉 microdesc；RENDEZVOUS1 用引言点 enc-key 并安装服务端 HS hop。
- **已做（真网观察，2026-09-20）**：同上 WORKING 行。未把主机名、公网 IP、中继指纹写入本文。
- **要做**：官方客户端交叉访问；prop 362 控制环。在此之前 **禁止** 描述符写 `HSDir=` / `HSIntro=` / `HSRend=`。
- **禁止**：`TestHandleIntroduce2` 的 mock 字节当 WORKING；明文上传描述符；全零 intro/ntor key。把 ESTABLISH 成功单独写成托管上线。

### 8. extra-info 完整

- [ ] **状态**：PARTIAL（digest 交叉引用 + 观测带宽历史已接线，含出站中间跳 OR 与 **ipv6-*-history**；描述符 **bandwidth observed 来自已完成格，无观测为 0**；**conn-bi-direct / ipv6-conn-bi-direct 满 24h 才写**；**dirreq-v3-resp / dirreq-v3-ips / dirreq-v3-reqs / dirreq-v3-direct-dl / dirreq-v3-tunneled-dl 满 24h 才写**；ips/reqs 仅 `??`；dl 含 complete/timeout/running；**exit-* 满 24h 且有出口 TCP 才写**；**hidserv-v3-* 满 24h 且有会合转发或 HSDir 接受才写（Laplace 混淆）**；**无** dl 分位数、geoip 国家码，**无**真网权威归档观察）
- **现有代码**：`GenerateDescriptorPair`（`pkg/relay/descriptor.go`）、`BandwidthHistory`（`pkg/relay/bw_history.go`）、`HidservStats`（`pkg/relay/hidserv_stats.go`）、`PublishDescriptorPair`（`pkg/relay/publisher.go`）。互操作 `docs/interop/extra-info.md`。
...
- **已做（协议切片，2026-09-19）**：`ipv6-write-history` / `ipv6-read-history` 分计 IPv6 OR（IPv4-mapped 不算）；与总量同一 900s 轴；无 IPv6 字节不写。读写 C Tor `BWHistoryIPv6*`。
- **已做（协议切片，2026-09-20）**：描述符 `bandwidth` 第三个数为已完成 900s 格 `min(max(read/s), max(write/s))`，且 ≤ burst；无观测写 0，不用配置平均冒充。
...
- **已做（协议切片，2026-09-19）**：`dirreq-v3-direct-dl` / `dirreq-v3-tunneled-dl` 写 `complete`（200 且未满 10 分钟）、`timeout`（开始发送后 10 分钟未完成）、`running`（测量期末仍在传且未满 10 分钟）。无速率观测，不写分位数。
...
- **已做（协议切片，2026-09-20）**：`hidserv-v3-stats-end` / `hidserv-rend-v3-relayed-cells` / `hidserv-dir-v3-onions-seen`。会合点拼接后转发的 RELAY 格与 HSDir 接受的唯一盲化公钥；满 24h 且有观测才写；先向上取 bin 再加 Laplace 噪声（可负）；同一窗不重新采样。无 v2 观测不写旧 `hidserv-*` 行。不读共识 hidserv-stats-* 覆盖参数。
- **要做**：geoip 国家码、dl 分位数、真网权威归档。不要把空统计当完整。
...
- [ ] **状态**：PARTIAL（客户端与托管 intro/rend HS 电路固定 L2+L3 并落盘；读共识 `guard-hs-l2-*` / `guard-hs-l3-*`。托管已真网 HTTP 200，选路走同一套 vanguards；未核对 C Tor `Guard in=` 行格式）
- **现有代码**：`pkg/path/vanguards.go`；`pkg/onion/hs_path.go`（`CircuitAdapter` / `BegindirFetcher` / SOCKS / **托管 intro+rend**）；`pkg/circuit/builder.go`（`Path.Middle2`）。互操作 `docs/interop/vanguards-lite.md`。
...
- **已做（协议切片，2026-09-20）**：托管 intro（`IntroPointManager.selectIntroPath`）与 rend（`RendezvousCircuitBuilder.selectPathToRelay`）走同一套 `selectOnionPath`；末跳必须是引言点/会合点。`startConfiguredOnionServices` 注入客户端 `VanguardSet`；描述符上传 BEGIN_DIR 同样走 vanguards。已注入则失败关闭。
- **要做**：真网托管上线后确认 intro/rend 四跳；与 C Tor 完全相同的 state `Guard in=...` 行格式。
- **禁止**：随机多跳冒充 vanguards；无持久化状态就宣称已防护；把本切片写成插件级完整 vanguards。
...
- [x] **状态**：WORKING（hspow-spec v1 Equi-X：解析 `pow-params v1` + 纯 Go HashX/Equi-X + INTRODUCE1 内层 EXT 0x02 + 托管 INTRODUCE2 校验。真网：独立 ClientOnly 进程开启 `HiddenServicePoWDefensesEnabled`，SOCKS 访问 `h22s2d4irewv7f7yokwovjd2tsyf3pfwhqyshe3wkjlv32saryvxljyd.onion` **HTTP 200**（`gotor-hs-ok`），日志 `Onion PoW solved effort=1` 与 `INTRODUCE2 PoW verified effort=1`。**无** prop 362 控制环）
- **现有代码**：`pkg/crypto/hashx/`、`pkg/crypto/equix/`、`pkg/onion/pow.go`（`ConnectToOnionService` / `parseDecryptedLayer` / `HandleIntroduce2`）。互操作 `docs/interop/hs-pow.md`。
...
- **已做（协议切片，2026-09-20）**：`HiddenServicePoWDefensesEnabled`（默认关）；描述符第二层写 `pow-params v1`；INTRODUCE2 内层按现行 rend-spec 解析 EXT 0x02；Equi-X 验证 + 工作量；接受当前或上一轮种子。suggested-effort 固定为 1。无队列速率自适应。
- **已做（真网观察，2026-09-20）**：描述符 `pow_enabled=true pow_effort=1`；客户端选到含 PoW 的描述符；Equi-X 求解；INTRODUCE2 校验通过后会合 BEGIN。HSDir 拉取取最高 `revision-counter`，避免过期描述符挡住 EXT 0x02。未把主机名、公网 IP、中继指纹写入本文。
- **要做**：prop 362 控制环（P2）。
- **禁止**：把无 PoW 的 `.onion` HTTP 200 写成 PoW WORKING；链接 LGPL Equi-X C / CGO；把 prop 362 队列自适应写成已完成。

### 13. HTTP CONNECT（HTTPTunnelPort）

- [x] **状态**：WORKING（C Tor `HTTPTunnelPort`：只接受 CONNECT；`RELAY_CONNECTED` 之后才 `HTTP/1.0 200 Connection established`；失败 403/502/504。真网：`curl -x` HTTP 隧道访问 `https://check.torproject.org/api/ip` 返回 `IsTor=true`。日志 `HTTP CONNECT` 后 `HTTP CONNECT established`。不接受 GET。Arti prop 365 扩展头未做。）
- **现有代码**：`pkg/httptunnel/httptunnel.go`、`pkg/socks/dial.go`、`pkg/circuit/stream_conn.go`、`pkg/client/dropin.go`、`pkg/config/loader.go`
- **已做（协议切片，2026-09-20）**：拨号超时只约束 BEGIN→CONNECTED；成功后 Read 不随 dial ctx 取消。出口走电路池 `GetIf` + `replaceIfExitRejected`；`.onion` 走 `ConnectToOnionService`。`unix:` 监听；MapAddress / SafeSocks / ClientRejectInternalAddresses 与 SOCKS 共用。
- **已做（真网观察，2026-09-20）**：独立 ClientOnly 进程加 `HTTPTunnelPort`（未改公开中继 torrc）。HTTPS CONNECT 得 `IsTor=true`。未把主机名、公网 IP、中继指纹、出口 IP 写入本文。
- **要做**：Arti prop 365 扩展头（P2）。`.onion` 经 CONNECT 已接线，本次 HSDir 拉取超时未记洋葱隧道 WORKING。
- **禁止**：OpenStream 之前写 200；把 SOCKS `IsTor=true` 写成 HTTPTunnel WORKING；把 Arti 扩展头写成已完成。
...
| **洋葱 PoW / 反 DoS**（[prop 327](https://spec.torproject.org/proposals/327-pow-over-intro.html)、[prop 362](https://spec.torproject.org/proposals/362-update-pow-control-loop.html)） | 1.3.x 设计/铺地；1.4.6 换成 prop 362 控制环；1.5.0 实验支持（[!3106](https://gitlab.torproject.org/tpo/core/arti/-/merge_requests/3106)）。稳定化仍开放：[arti#1751](https://gitlab.torproject.org/tpo/core/arti/-/issues/1751) | 有（0.4.8 `HiddenServicePoW*`，默认关） | v1 Equi-X **WORKING**（真网求解+校验）；prop 362 **MISSING** | `pkg/crypto/hashx`、`pkg/crypto/equix`、`pkg/onion/pow.go` | **P1** v1 已真网验收。**P2** prop 362 控制环。未 required |
| **Vanguards-lite** | **1.2.2 默认** lite（[#1272](https://gitlab.torproject.org/tpo/core/arti/-/issues/1272) 等）；1.2.3 修 TROVE-2024-003 / [arti#1409](https://gitlab.torproject.org/tpo/core/arti/-/issues/1409)（电路少一跳） | 有（默认 lite；完整 L3 为插件/完整 vanguards） | **PARTIAL**（客户端+托管 intro/rend 固定 L2+L3 且读共识 `guard-hs-l*`；托管已真网 HTTP 200） | `pkg/path/vanguards.go`、`pkg/onion/hs_path.go` | **P1** 客户端与托管选路已接线。见上文清单第 10 项 |
...
| **HTTP CONNECT 代理**（[prop 365](https://spec.torproject.org/proposals/365-http-connect-ext.html)） | 1.7.0 实验 `http-connect`；**2.2.0 稳定且默认开** | 有（`HTTPTunnelPort`） | C Tor HTTPTunnelPort **WORKING**（真网 CONNECT + `IsTor=true`）；Arti 扩展头 **MISSING** | `pkg/httptunnel/httptunnel.go`、`pkg/socks/dial.go` | **P1 已跟 C Tor**。Arti 扩展头 **P2** |
