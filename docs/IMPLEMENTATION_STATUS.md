...
| Relay / Bridge | PARTIAL | ORPort；入站握手 VERSIONS(2)→CERTS 1/2/4/5/7→AUTH_CHALLENGE→NETINFO（协商后 CircID=4）；ORPort self-test 门闩（未测活不发布，`AssumeReachable` 跳过）；中间跳出站握手（VERSIONS/CERTS/NETINFO）+ CircID MSB + 按身份入池 + 剥层/回程（离线单测，无官方客户端经 middle 出网证据）；DirCache 可提供 consensus-microdesc / micro/all / `/tor/keys` / **上一份→当前 limited-ed**（未宣告 DirCache=2）；末端跳 ESTABLISH_INTRO / ESTABLISH_RENDEZVOUS 回 ACK（未宣告 HS*；无 HSDir / INTRODUCE1 / RENDEZVOUS1）；**ExitRelay 1 出口流**；CREATE2 ntor/ntor-v3；描述符交叉证书与 Ed25519 摘要签名。未做：真网 Running / 进共识、AUTHENTICATE 校验、网桥、真网多跳中继/出口收录验收 |
