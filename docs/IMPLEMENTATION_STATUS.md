...
| Relay / Bridge | PARTIAL | ORPort；入站握手 VERSIONS(2)→CERTS 1/2/4/5/7→AUTH_CHALLENGE→NETINFO（协商后 CircID=4）；**LinkAuth=3 校验 AUTHENTICATE type 3**；ORPort self-test 门闩（未测活不发布，`AssumeReachable` 跳过）；中间跳出站握手（VERSIONS/CERTS/NETINFO）+ CircID MSB + 按身份入池 + 剥层/回程（离线单测，无官方客户端经 middle 出网证据）；DirCache 可提供 consensus-microdesc / micro/all / `/tor/keys` / **上一份→当前 limited-ed** / **gzip/deflate/`.z` / 304** / **FPRLIST 签名过滤**（未宣告 DirCache=2）；末端跳 ESTABLISH + INTRODUCE1→INTRODUCE2 / RENDEZVOUS1→RENDEZVOUS2 + HSDir `/tor/hs/3` 验签收/服（未宣告 HS*；无限速/哈希环/真网被选）；**ExitRelay 1 出口流**；CREATE2 ntor/ntor-v3；**ntor-v3 type 3 `[02 06]` 则 CGO 剥层/回程**（未宣告 Relay=5-6）；描述符交叉证书与 Ed25519 摘要签名；**extra-info-digest + 观测带宽历史**（无观测不写 history）；**官方 DoS* 键 + CREATE2/每 IP 接线**（默认 auto 关，无共识参数）。未做：真网 Running / 进共识、网桥、真网多跳中继/出口收录验收 |
