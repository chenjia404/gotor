...
| Control Protocol | WORKING | GETINFO/SETCONF/SETEVENTS/SIGNAL/MAPADDRESS；**GETINFO version=`Tor 0.4.9.11 (gotor)`**；**GETINFO traffic/read、traffic/written 为入口 OR TLS 累计字节**；**GETINFO status/enough-dir-info 依验签共识与选路表**；**GETINFO net/listeners/socks 与 control 为实际绑定**；**GETINFO config-file 为 torrc 路径**；**AUTHCHALLENGE SAFECOOKIE**；COOKIE/HASHEDPASSWORD；事件 CIRC/STREAM/BW/…/NOTICE |
