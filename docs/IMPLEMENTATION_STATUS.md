...
| 15 | 共识只数签名个数 | `VerifyConsensusSignatures` 从未被 `FetchConsensus` 调用；证书取第一把 RSA（identity） | dir-spec consensus-formats / authority-key-certificates；C Tor signed boundaries |
...
| `integration/e2e_real_tor_test.go` | 共识验签 / CREATE2 / 3-hop / check.torproject.org | 同上 |
