...
| Onion Service v3 | WORKING | 客户端路径真实验收：描述符→会合→hs-ntor→BEGIN→**HTTP 200**（Tor Project onion，~14KB）。**vanguards**：HS 电路 L1→固定 L2→固定 L3→目标并落盘，读共识 `guard-hs-l*`；**托管 intro/rend 同样走 L2+L3**（托管仍未真网上线）。**PoW** PARTIAL：`pow-params v1` + Equi-X + INTRODUCE1/2 EXT 0x02（无真网 PoW 服务验收；无 prop 362） |
| Onion Service v3（托管） | PARTIAL | ESTABLISH_INTRO；ntor rend_circ_nonce；BEGIN_DIR 上传；**type-8 致盲证书 + 双层加密密封**；torrc HiddenService*；**intro/rend vanguards L2+L3**；**HiddenServicePoWDefensesEnabled 写 pow-params 并校验 INTRODUCE2**；**ESTABLISH 后常驻收 INTRODUCE2**；上传与引言点池已拆开。**真网 ESTABLISH_INTRO 成功**；**HSDir POST 400 未上环**。剩余：描述符被真网 HSDir 收下并完成 INTRODUCE2 回路 |
