...
| Onion Service v3 | WORKING | 客户端路径真实验收：描述符→会合→hs-ntor→BEGIN→**HTTP 200**（Tor Project onion，~14KB）。**vanguards**：HS 电路 L1→固定 L2→固定 L3→目标并落盘，读共识 `guard-hs-l*`；**托管 intro/rend 同样走 L2+L3**。**PoW** PARTIAL：`pow-params v1` + Equi-X + INTRODUCE1/2 EXT 0x02（无真网 PoW 服务验收；无 prop 362） |
| Onion Service v3（托管） | WORKING | 独立 ClientOnly 进程（未改公开中继 torrc）。HSDir POST **200**；`h22s2d4irewv7f7yokwovjd2tsyf3pfwhqyshe3wkjlv32saryvxljyd.onion` SOCKS **HTTP 200**（`gotor-hs-ok`）。ESTABLISH_INTRO → INTRODUCE2 → RENDEZVOUS1/2 → BEGIN/DATA。未宣告 HS*。PoW 默认关。 |
