...
| Onion Service v3 | WORKING | 客户端路径真实验收：描述符→会合→hs-ntor→BEGIN→**HTTP 200**（Tor Project onion，~14KB）。**vanguards**：HS 电路 L1→固定 L2→固定 L3→目标并落盘，读共识 `guard-hs-l*`；仍无托管侧。**PoW 客户端** PARTIAL：`pow-params v1` + Equi-X + INTRODUCE1 EXT 0x02（无真网 PoW 服务验收） |
