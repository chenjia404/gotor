...
| Onion Service v3（托管） | PARTIAL | ESTABLISH_INTRO；ntor rend_circ_nonce；BEGIN_DIR 上传；**type-8 致盲证书 + 双层加密密封**；torrc HiddenService*；**intro/rend vanguards L2+L3**；**HiddenServicePoWDefensesEnabled 写 pow-params 并校验 INTRODUCE2**；**ESTABLISH 后常驻收 INTRODUCE2**；上传与引言点池已拆开。剩余：真网发布/INTRODUCE2 回路验收 |
