...
| Onion Service v3 | WORKING | 客户端路径真实验收：描述符→会合→hs-ntor→BEGIN→**HTTP 200**（Tor Project onion，~14KB）。**vanguards**：HS 电路 L1→固定 L2→固定 L3→目标并落盘，读共识 `guard-hs-l*`；**托管 intro/rend 同样走 L2+L3**。**PoW v1 WORKING**：`pow-params v1` + Equi-X + INTRODUCE1/2 EXT 0x02，真网 effort=1 求解并校验（无 prop 362） |
| Onion Service v3（托管） | WORKING | 独立 ClientOnly 进程（未改公开中继 torrc）。HSDir POST **200**；`h22s2d4irewv7f7yokwovjd2tsyf3pfwhqyshe3wkjlv32saryvxljyd.onion` SOCKS **HTTP 200**（`gotor-hs-ok`）。ESTABLISH_INTRO → INTRODUCE2 → RENDEZVOUS1/2 → BEGIN/DATA。开启 `HiddenServicePoWDefensesEnabled` 时日志 `INTRODUCE2 PoW verified`。未宣告 HS*。 |
| Relay / Bridge | PARTIAL | ORPort；入站握手 VERSIONS(2)→CERTS 1/2/4/5/7→AUTH_CHALLENGE→NETINFO（协商后 CircID=4）；**LinkAuth=3 校验 AUTHENTICATE type 3，AUTH_CHALLENGE 只广告方法 3**；ORPort self-test 门闩（未测活不发布，`AssumeReachable` 跳过）；中间跳出站握手（VERSIONS/CERTS/NETINFO）+ CircID MSB + 按身份入池 + 剥层/回程（离线单测，无官方客户端经 middle 出网证据）；DirCache 可提供 **ns 与 microdesc 分库** / micro/all / `/tor/keys` / **最多 72 小时历史→当前 limited-ed** / **gzip/deflate/`.z` / 304** / **FPRLIST 签名过滤** / **x-zstd / x-tor-lzma** / **预压缩 consdiff 库**（未宣告 DirCache=2）；末端跳 ESTABLISH + INTRODUCE1→INTRODUCE2 / RENDEZVOUS1→RENDEZVOUS2 + HSDir `/tor/hs/3` 验签收/服与 **哈希环 spread_store 责任** + **引言点 DOS_PARAMS/共识令牌桶** + **会合点仅末跳/未会合 TTL**（未宣告 HS*；extra-info `hidserv-v3-*` 满 24h 才写，仍无真网被选）；**ExitRelay 1 出口流**；CREATE2 ntor/ntor-v3；**ntor-v3 type 3 `[02 06]` 则 CGO 剥层/回程 + 出口 SENDME v1（16 字节 tag FIFO）+ FlowCtrl=2 TOR_VEGAS**（未宣告 Relay=5-6）；描述符交叉证书与 Ed25519 摘要签名；**proto 只宣告 Cons/Desc/Microdesc/Link/LinkAuth/Relay/FlowCtrl**（无 Circuit/Padding/Conflux/DirCache=2/HS*/Relay=5-6）；**extra-info-digest + 观测带宽历史**（入站 OR + 出站中间跳；IPv6 另写 ipv6-*-history；无观测不写 history）+ **bandwidth observed 来自已完成格**+ **conn-bi-direct / ipv6-conn-bi-direct（满 24h 才写）** + **dirreq-v3-resp / dirreq-v3-ips / dirreq-v3-reqs / dirreq-v3-direct-dl / dirreq-v3-tunneled-dl（满 24h 才写；ips/reqs 仅 ??；dl 含 complete/timeout/running）** + **exit-*（满 24h；BEGIN_DIR 不计）** + **hidserv-v3-*（满 24h 且有观测才写；Laplace 混淆）**；**官方 DoS* 键 + CREATE2/每 IP + auto 跟共识 + ConnectRate/Burst + StreamCreation + AUTHENTICATE 单跳区分 + CircuitCreationDefenseType + 共识 nodelist 核对身份**（默认 auto 关）。未做：真网 Running / 进共识、网桥、真网多跳中继/出口收录验收 |
| Control Protocol | WORKING | GETINFO/SETCONF/SETEVENTS/SIGNAL/MAPADDRESS；**GETINFO version=`Tor 0.4.9.11 (gotor)`**；**GETINFO traffic/read、traffic/written 为入口 OR TLS 累计字节**；**GETINFO status/enough-dir-info 依验签共识与选路表**；**GETINFO net/listeners/socks、control、httptunnel、dns、or、dir 为实际绑定**；**GETINFO config-file 为 torrc 路径**；**GETINFO config-text 为 dump-config short**；**GETINFO circuit-status 真网 BUILT（`$` hex~nick、PURPOSE、TIME_CREATED）**；**GETINFO ns/id 真网 250+ r/s/w**；**GETINFO desc/id 无 server descriptor 时 551**；**GETINFO stream-status** 列出 SOCKS/HTTP CONNECT 活动流；**GETCONF CircuitPadding/SafeSocks/ConnectionPadding 为当前值**；**GETCONF ClientUseIPv4/IPv6 与 ClientRejectInternalAddresses 为当前 0/1**；**GETCONF HTTPTunnelPort/DNSPort 为当前端口**；**GETCONF DisableNetwork/ClientOnly 为当前 0/1**；**GETCONF ORPort/DirPort/Nickname/ExitRelay 为当前中继配置**；**GETCONF ContactInfo/Address/PublishServerDescriptor/AssumeReachable 为描述符发布相关配置**；**GETCONF DirCache/IPv6Exit/ReduceExitPolicy 为当前 0/1（DirCache 是 torrc 开关，不是协议版本 2）**；**GETCONF BandwidthRate/Burst 为字节数（0=不限）**；**GETCONF ExitPolicy 为 torrc 显式规则（不含默认策略展开）**；**GETCONF MyFamily/FamilyID 为当前家族配置**；**AUTHCHALLENGE SAFECOOKIE**；COOKIE/HASHEDPASSWORD；事件 CIRC/STREAM/BW/…/NOTICE |
...
- **中继（#59）**：可识别 ntor-v3 type 3 `[02 06]` 并 ENC_UIV 剥层/回程；出口电路级 SENDME v1（16 字节 CGO tag FIFO）；**出口 FlowCtrl=2 TOR_VEGAS**（无真网出口 soak）；**未**宣告 `Relay=5-6`；无真网被请求证据。见 `docs/interop/cgo-relay.md`。
...
| 项 | 原因 | 已知坑 |
|----|------|--------|
| 10 | Onion Service v3（托管） | PARTIAL | ESTABLISH 后常驻收 INTRODUCE2；上传走 HSDir 哈希环；真网发布/接入仍待验收 |
...
### Link VERSIONS CircID 宽度 — 已修（出站 + 入站）
...
- 入站应答：VERSIONS → CERTS（type 1/2/4/5/7，type 4 含 signed-with-ed25519-key，type 5 绑 SHA-256(TLS 证)）→ AUTH_CHALLENGE（仅方法 3）→ NETINFO；发起方若发 CERTS+AUTHENTICATE 则按 LinkAuth=3 校验（清单第 5 项）。客户端只发 NETINFO。不广告未实现的 AuthType 1。
...
- **缺口**：官方 v0 relay-cell 向量；真网被请求 CGO；中继出口 Vegas 无真网 soak。
