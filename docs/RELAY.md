...
- DirPort / BEGIN_DIR 可服务已缓存的 `cached-microdesc-consensus` / `cached-microdescs` / `cached-certs`（`/tor/keys/fp`、`/tor/keys/all`）、上一份→当前 limited-ed、gzip/deflate/`.z`、If-Modified-Since 304 与 FPRLIST 签名过滤（未过半 404）；仍缺多小时历史 / 真网被当缓存，未宣告 DirCache=2
