...
- 官方 `DoS*`：默认 auto 关闭；auto 跟共识 `DoSCircuitCreationEnabled` / `DoSConnectionEnabled`。显式 1 时每 IP 并发 OR 上限 + CREATE2 令牌桶 + 连接速率桶（20/40/24h，torrc 0 跟共识）。`DoSRefuseSingleHopClient` 仅在成功 EXTEND 后放行 BEGIN。不改 `ConnLimit`。仍缺 StreamCreation。见 `docs/interop/dos-relay.md`。
