...
- extra-info：描述符写 `extra-info-digest`，与 extra-info 一次 POST；只写已完成 900s 观测格。入站 OR 与出站中间跳 OR 套接字计入 read/write-history。`conn-bi-direct` / `ipv6-conn-bi-direct` 满 24h 且有分类才写（后者仅 IPv6）。无观测不写 history / 双向行。仍缺 dirreq/exit/hidserv 与真网归档。
