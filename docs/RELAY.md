...
- 末端跳可受理 ESTABLISH、INTRODUCE1→INTRODUCE2、RENDEZVOUS1 会合与 HSDir `/tor/hs/3` 验签收/服；DirCache 在共识哈希环就绪后按 spread_store 拒绝非责任 POST。引言点按 ESTABLISH_INTRO `DOS_PARAMS` 或共识 `HiddenServiceEnableIntroDoS*` 做 INTRODUCE2 令牌桶。会合点仅末跳、未会合 cookie 10 分钟 TTL。extra-info `hidserv-v3-*` 满 24h 且有会合转发或 HSDir 接受才写（Laplace 混淆）。仍缺真网被选，未宣告 HS*
- 入站可校验 AUTHENTICATE type 3（LinkAuth=3）；AUTH_CHALLENGE 只广告方法 3；AuthType 1 拒绝。普通客户端不认证。
...
- extra-info：描述符写 `extra-info-digest`，与 extra-info 一次 POST；只写已完成 900s 观测格。入站 OR 与出站中间跳 OR 套接字计入 read/write-history；IPv6 OR 另写 `ipv6-*-history`（IPv4-mapped 不算，无 IPv6 字节不写）。描述符 `bandwidth` 第三个数来自已完成格 `min(max read/s, max write/s)`，无观测为 0。`conn-bi-direct` / `ipv6-conn-bi-direct` 满 24h 且有分类才写（后者仅 IPv6）。`dirreq-v3-resp` 满 24h 且有 v3 网络状态应答才写。`dirreq-v3-ips` / `reqs` 无 geoip 只写 `??=N`（向上取 8）。`dirreq-v3-direct-dl` / `tunneled-dl` 满 24h 写 `complete`/`timeout`/`running`（DirPort vs BEGIN_DIR；无分位数）。`exit-*` 满 24h 且有出口 TCP 观测才写（BEGIN_DIR 不计）。`hidserv-v3-*` 满 24h 且有会合转发或 HSDir 接受才写（先向上取 bin 再加 Laplace；无 v2 行）。无观测不写 history / 双向行 / dirreq / exit / hidserv。仍缺真实国家码、dl 分位数与真网归档。
